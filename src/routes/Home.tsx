...
export default function Home() {
  ...
  return (
    <>
      {/* HERO SECTION */}
      <section className="relative overflow-hidden rounded-3xl border border-brand-mist/20 bg-brand-slate/40">
        ...
        <div className="relative px-6 py-14 sm:px-10 sm:py-16 lg:px-16 lg:py-20">
          ...
          >
            <li>• Built an admin portal that reduced manual workload by <strong className="text-brand-fog">30%+</strong>.</li>
            <li>• Published a Unity game with <strong className="text-brand-fog">1,000+ downloads</strong>.</li>
            <li>• Skilled in <strong className="text-brand-fog">React, TypeScript, Node/Express, MongoDB/SQL</strong>.</li>
          </motion.ul>
          ...
          >
            ...
            <div className="ml-1 flex items-center gap-3 text-brand-mist">
              ...
              <a href="mailto:" className="hover:text-brand-fog" aria-label="Email">
                <Mail size={20} />
              </a>
            </div>
          </motion.div>
          ...
          >
            {[
              { title: 'Web & APIs', items: 'React, TypeScript, Node.js, Express, REST, SQL/MongoDB' },
              { title: 'Game & 3D', items: 'Unity (C#), Blender, Animations, Gameplay Systems' },
              { title: 'Ops & Quality', items: 'Git/GitHub, Docker, Jira, Testing, Performance' }
            ].map((b) => (
              <div key={b.title} className="rounded-2xl border border-brand-mist/20 bg-brand-base/50 p-4">
          ...
          <div className="grid md:grid-cols-2">
            ...
            <div className="relative h-60 md:h-full">
              {featured.visuals?.banner ? (
                <ParallaxBanner layers={[{ image: featured.visuals.banner, speed: -20 }]} className="absolute inset-0 h-full w-full" />
              ) : (
            ...
            <div className="p-6 md:p-8 lg:p-10">
              ...
              <h2 className="mt-3 text-2xl md:text-3xl font-bold text-brand-fog">{featured.title}</h2>
              <p className="mt-2 text-brand-mist">{featured.summary}</p>

              {/* Tech Logos */}
              <div className="mt-3 flex flex-wrap gap-2">
                <TechLogos items={toIconItems(featured.stack)} />
              </div>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                {featured.links?.repo && (
                  <a href={featured.links.repo} target="_blank" rel="noreferrer"
                     className="inline-flex items-center gap-2 rounded-2xl border border-brand-mist/30 bg-brand-base/50 px-5 py-3 text-brand-fog hover:bg-brand-base/70">
                    GitHub
                  </a>
                )}
